...
const batchesDir = path.join(__dirname, '../questions-data/agk_batches');

for (let i = 1; i <= 13; i++) {
  const batchFile = path.join(batchesDir, `batch_${i}_done.json`);
...
console.log(`Loaded ${Object.keys(explanationsMap).length} explanations from batch done files.`);

// ── Parse Original AGK.csv ──────────────────────────────────
const csvFilePath = path.join(__dirname, '../questions-data/AGK.csv');
const rows = [];
...
fs.createReadStream(csvFilePath)
  ...
  .on('end', async () => {
    console.log(`Parsed ${rows.length} rows from AGK.csv`);
    ...
    const outputCsvPath = path.join(__dirname, '../questions-data/AGK_temp.csv');
    fs.writeFileSync(outputCsvPath, csvContent, 'utf-8');
    console.log(`Written completed CSV to: ${outputCsvPath}`);

    // ── Fetch Existing AGK Questions in Supabase ─────────────
    console.log('Fetching existing AGK questions from Supabase...');
    ...
    console.log(`Found ${existingQuestions.length} existing AGK questions in Supabase.`);
    ...
    console.log('🎉 Done uploading all AGK explanations to Supabase.');
  });
